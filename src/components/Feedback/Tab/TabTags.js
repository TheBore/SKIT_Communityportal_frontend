...
class TabTags extends Component {
    ...
    fillTagsList = () => {
        TagRepository.getAllTags().then(res => {
            let tmp = []
            for (let i = 0; i < res.data.length; i++) {
                tmp.push({value: res.data[i].id, label: res.data[i].tagNameMk, name: "tag"})
            }
            this.setState({
                tagsList: tmp
            })
    ...
    handleChange = (tags) => {
        if (tags) {
            ...
            tmp = tmp.toString();
            InstitutionRepository.allInstitutionsByTags(tmp).then(async res => {
                let dataTmp = []
                for (let i = 0; i < res.data.length; i++) {
    ...
    handleCheckboxInputChange = () => {
        if (this.state.isAllSelected === true) {
            ...
            for (let i = 0; i < this.state.tagsList.length; i++) {
                tmp.push(this.state.tagsList[i].value)
            }
            tmp = tmp.toString();
            InstitutionRepository.allInstitutionsByTags(tmp).then(async res => {
                let dataTmp = []
                    ...
                    <div className="row">
                        ...
                        {this.props.meeting === "Meeting" ?
                            <div className="col-md-4 text-right">
                                <button className="defaultBtn btn btn-info"
                                        onClick={() => this.publishToInstitutions()}>
