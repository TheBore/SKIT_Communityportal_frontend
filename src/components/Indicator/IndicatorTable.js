...
class IndicatorTable extends Component {
    ...
    onAddIndicator = (entity) => {
        entity["activity"] = this.state.activity.id;

        return IndicatorRepository.createIndicator(entity).then( async res => {
            toast.success(strings.addIndicatorSuccess);
            await this.sleep(2000);
            window.location.reload();
        }).catch( async err => {
            toast.error(strings.addIndicatorNoSuccess);
            console.log(err);
            await this.sleep(2000);
            window.location.reload();
        })
    ...
    render() {
        ...
        const indicatorValidations = (['nameMk', 'status', 'startDate', 'institution', 'type']);
                ...
                <div className="container-fluid">
                    <div className="row">
                        ...
                        <div className="col-md-3 text-right" style={{marginTop: "3.8rem"}}>
                            {localStorage.getItem('role') !== 'ROLE_EVALUATOR' ?
                                <div>
                                    ...
                                    <div>
                                        <CrudModal
                                            entity={{activityId: this.state.activityId}}
                                            icon={'add'}
                                            btnClass={'addIndicatorBtn'}
                                            title={strings.addIndicator}
                                            showText={true}
                                            onSubmit={this.onAddIndicator}
                                            validations={indicatorValidations}
                                            body={IndicatorAdd}
                                        />
                                    </div>
                                </div>
                        ...
                        <table className="table-hover newTable mt-2" style={{width: "100%", marginLeft: "13px"}}>
                            ...
                            <tr>
                                <th className="tableHeading firstHeading" style={{width: "12%"}}>{strings.name}</th>
                                <th className="tableHeading" style={{width: "10%"}}>{strings.dateStarted}</th>
                                <th className="tableHeading" style={{width: "12%"}}>{strings.responsibleInstitution}</th>
                                <th className="tableHeading" style={{width: "12%"}}>{strings.typeOfIndicator}</th>
                                <th className="tableHeading" style={{width: "12%"}}>{strings.statusOfFinished}</th>
                                <th className="tableHeading" style={{width: "12%"}}>{strings.statusOfCounter}</th>
                                <th className="tableHeading" style={{width: "13%"}}>{strings.statusOfIndicator}</th>
                                <th className="tableHeading" style={{width: "13%"}}>
                                    {strings.evaluationPlan}: {localStorage.getItem("activeLanguage") === 'mk' ? this.state.evaluation.descriptionMk : this.state.evaluation.descriptionAl}&nbsp;
                                    {this.state.nap.openForEvaluation ?
                                        <span className="badge badge-pill badge-danger">{strings.opened}</span> :
                                        <span className="badge badge-pill badge-primary">{strings.closed}</span>}
                                </th>
                                {/*<th className="tableHeading lastHeading">Статус на актуелен извештај</th>*/}
                            </tr>
                            </thead>
                            <tbody>
                            {this.state.indicators.map((item,index) => {
                                return(
                                    <Indicator activity={this.state.activity} indicator={item} nap={this.state.nap} key={item.id}/>
                                )
                            })}
